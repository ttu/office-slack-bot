...
class PlugIn {
    constructor() {
        this.keywords = [];
        this.handle = () => {};
        // Format:[keyowrd]       [description]
        this.helperText = '';
        this.helperVerboseText = '';
    }
}
...
class BotClass {
    constructor() {
        ...
        this.notifyFunc = (output) => {};
        this.PlugIns = [];
    }

    addPlugIn(plug) {
        var allKeyWords = this.PlugIns.reduce((p,c) => p.concat(c));
        var isValid = plug.keywords.reduce((result, keyword) => (!result) ? result : !allKeyWords.some(keyword), true);
        if (!isValid) throw ("Existing keywords not allowed");
    ...
    handle(message, caller) {
        ...
        var matches = this.PlugIns.filter(p => p.keywords.some(e => e === command));

        if (matches.some()) {
            return matches[0].handle(message, caller);
        } else {
            return this.createHelp(command, args);
        }
    }

    createHelp(command, args){
        if (command === 'help') {
            const help = `Options:\nhelp       View this message (see \`help verbose\` for more)`;

            let output = this.PlugIns.reduce((p, c) => `${p}\n\n${c.helperText}`, help);

            output = args[1] && args[1] == 'verbose' 
                        ? this.PlugIns.reduce((p, c) => `${p}\n\n${c.helperVerboseText}`, `${output}\n\n`)
                        : output;
